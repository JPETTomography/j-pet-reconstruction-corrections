...
void prepare_castor_files(string listName, int total_time)
{
    long int total = merge_header(listName, total_time);
    
    std::ifstream Cdf_file("merged.Cdf",std::ios::binary);
    std::ofstream new_Cdf_file("merged_timeCorrected.Cdf",std::ios::binary);
...
int merge_header(string listName, int total_time)
{
    ...
    std::ifstream Cdh_file(fileName);
    std::ofstream file("merged_timeCorrected.Cdh");
    
    getline(Cdh_file, line);
    file << line << endl;
    
    getline(Cdh_file, line);
    file << "Data filename: merged_timeCorrected.Cdf" << endl;
